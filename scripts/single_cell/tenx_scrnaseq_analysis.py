...
def parseArgs():
    ...
    # Defining minimum read counts for each cell
    try:
    ...
    # Defining minimum number of detected genes for each cell
    try:
        
        min_genes = int(argv[argv.index("--min_genes") + 1])

    except:
        
        min_genes = 500

    # Reading user-defined n_neighbors
    try:
        
        neighbors = [int(n) for n in argv[argv.index("--n_neighbors") + 1].split(',')]

    except:
        
        neighbors = [5, 30, 50]
    
    return input_dir, min_counts, min_genes, neighbors
...
input_dir, min_counts, min_genes, neighbors = parseArgs()

# Load and normalize data
data = tenx_scranseq()
data.importRawData(input_dir = input_dir, min_counts = min_counts, min_detected_genes = min_genes)
data.normalizeCounts()

# Finding most variable genes
data.findMostVariableFeatures(x_low_cutoff = 0.1, x_high_cutof = 8, y_low_cutoff = 1, y_high_cutoff = 1e9, nbins = 20, max_features = 2000)

# Scale data prior to PCA
data.scaleFeatures()

# Generate UMAPs
data.reduceDimensions(features='variable_features', pca_components = 50, neighbors = neighbors, save_model = True)

### CLUSTERING ----------------------------- ###

# Clustering with Leiden algorithm on top PC
data.clusterCells(n_neighbors=10)
data.plotClusters(dot_size=15)

# Finding cluster markers
data.findClustersMarkers(min_fc=0.25, min_pct=0.1)

### SAVING DATA ---------------------------- ###

data.saveClassObject(object_name="scRNASeq_Analysis")
